...
"black"))+
  ...
  stat_bin(binwidth = n,boundary = 0, aes(label=ifelse(..count.. > 0, ..count.., "")),geom="text",vjust=-0.9,hjust=0.5)+ coord_cartesian(ylim=c(0, max_count + 30))#+labs(caption="Binwidth is 5.")
...
max_value <- max(hist(reads3$value, breaks=seq(0, max(reads3$value) + 5, by=5), plot=FALSE)$counts) 
...
element_line(colour = "black"))+
  # update: adds counts above bars
  stat_bin(binwidth = 5,boundary = 0, aes(label=ifelse(..count.. > 0, ..count.., "")),geom="text",vjust=-0.9)+ coord_cartesian(ylim=c(0, max_value + 30))#+labs(caption="Binwidth is 5.")

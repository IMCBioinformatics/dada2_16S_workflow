...
files <- list.files(path = snakemake@input[["quality"]], pattern = "raw", full.names = TRUE)
...
files <- list.files(path = snakemake@input[["quality"]], pattern = "afterQC", full.names =TRUE)
...
files <- list.files(path = snakemake@input[["quality"]], pattern = "afterdada2", full.names =TRUE)
...
files <- list.files(path = snakemake@input[["Length_distribution"]], pattern = "S1", full.names =TRUE)
...
files <- list.files(path = snakemake@input[["Length_distribution"]], pattern = "S2", full.names =TRUE)
...
files <- list.files(path = snakemake@input[["Length_distribution"]], pattern = "S3", full.names =TRUE)
...
files <- list.files(path = snakemake@input[["Length_distribution"]], pattern = "S4", full.names =TRUE)
...
files <- list.files(path = snakemake@input[["Length_distribution"]], pattern = "S5", full.names =TRUE)
...
reads<-read.csv(snakemake@input[["Nread"]],sep="\t") %>% filter(!grepl('Undetermined|undetermined', file))
...
ggplot(reads2, aes(x=value)) + 
  geom_histogram(color="black", fill="#E69F00",binwidth = 25000,boundary=0)+   #boundary start starts the bin from zero
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, size = 12, colour = "black",  hjust = 1, face= "bold"),
        axis.title.y = element_text(size = 12, face = "bold"), legend.title = element_text(size = 16, face = "bold"),
        axis.title.x = element_text(size = 12, face = "bold"),
...
"black"))+
  ...
  stat_bin(binwidth = 25000,boundary = 0, aes(label=ifelse(..count.. > 0, ..count.., "")),geom="text",vjust=-0.9,hjust=0.5)#+labs(caption="Binwidth is 5.")

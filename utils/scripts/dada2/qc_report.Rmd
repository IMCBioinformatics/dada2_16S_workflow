...
# Read data
seqtab <- data.frame(readRDS(file = snakemake@params[["seqtab"]]))
otu_df <- seqtab
...
ggplot(df, aes(x = Abundance, y = Prevalence)) + 
  ...
  ggtitle("A Glimpse at ASVs through Prevalence and Abundance")+
  labs(caption = paste0("Points in green represent ASVs with prevalence > 10 and abundance > 150. ", 
"Total number of ASVs is ", length(df$Taxa), " in ", nrow(otu_df) ," samples.")) +

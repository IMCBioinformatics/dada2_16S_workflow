...
rule cutadapt_qc:
    ...
    output:
        R1= "output/cutadapt_qc/{sample}_R1.fastq.gz",
        R2= "output/cutadapt_qc/{sample}_R2.fastq.gz"
    ...
    log: 
        "output/logs/qc/cutadapt_primer/cutadapt{sample}.log"
    shell:
        "cutadapt -q {params.qf},{params.qr} -o {output.R1} -p {output.R2} {input.R1} {input.R2} >> {log}"
#	 " {params.inputs}"
